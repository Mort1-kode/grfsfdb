...
// //const cleanedstrings som en tom liste, hvor vi legger de ryddige ordene oppi.
// function cleanText(strings) {
//   const cleanedStrings = [];

//   //.trim til og fjerne whitespace, to lowercase for og få alt i små bokstaver
...
//   " Up",
// ];
// const result = cleanText(inputArray);
// console.log(result);
...
function helloChecker(greeting) {
  ...
  //bruker denne til og gjør alt om til små bokstaver, for og få svar om man bruker både Ciao og ciao, split for og dele opp ord
  const words = greeting.toLowerCase().split(" ");

  for (const word of words) {
    for (const [greetingWord, language] of Object.entries(greetings)) {
      if (word === greetingWord) {
...
// skriv inn ordet på hvilket språk her for og consol-logge hello. eks salute = not hello detected.
const result = helloChecker("Salute");
console.log(result);
/******************************************************************************
5.
